...
static loff_t  dm_lseek(struct file *filp, loff_t offset, int origin);
...
static inline ssize_t writeMem(struct file *filp, const char *buf, size_t count, loff_t *f_pos)
{
	...
	if(mem_to_write->seeked > 0){
                (*f_pos) = (*f_pos) + mem_to_write->seeked;
                mem_to_write->seeked = 0;
        }
	...
	if (copy_from_user(mem_to_write->dma.buf, buf, transfer_size)) {
		DBG_LOG("Copy from user space failed \n");
		return -EFAULT;
	}
...
static inline ssize_t readMem(struct file *filp, char *buf, size_t count, loff_t *f_pos)
{
	...
	if(mem_to_read->seeked > 0){
		(*f_pos) = (*f_pos) + mem_to_read->seeked;
		mem_to_read->seeked = 0;
	}
#ifdef USE_WORD_ADDRESSING
	if (count % 2 != 0) {
	...
	while (transferred < count) {
		...
		if (copy_to_user(&buf[transferred], mem_to_read->dma.buf, transfer_size)) {
			DBG_LOG("Copy to user space failed \n");
			return -EFAULT;
		}
...
static loff_t  dm_lseek(struct file *filp, loff_t offset, int origin){
	struct drvr_mem * mem_to_seek = &(((struct drvr_device *) filp->private_data)->data);
	mem_to_seek->seeked += (loff_t) offset ;
	//DBG_LOG("seeked to  %llxn \n", offset);
	return offset ;
}
